...
LEARNING_RATES = [0.000005]  # [0.00001, 0.0001, 0.001, 0.01, 0.1, 0.5]
HIDDEN_LAYER = 100

RELOAD_MODEL = True
TRAIN_MODE = True
SAVE_PLOT = True
LOG_PATH = '../summary/problemA4'
    ...
    def replay(self, LEARNING_RATE):
        ...
        with tf.Session() as sess:
            ...
            if TRAIN_MODE:
                ...
                for ii in range(100):
                    ...
                    for itr in range(N_EPISODES):
                        ...
                        for t in range(self.MAX_EPISODE_LEN):
                            ...
                            max_next_Q, next_Q = self.brain.predict_Q(sess, np.expand_dims(next_state, axis=0))

                            Q_target = np.copy(Q)
                            Q_target[0, action[0]] = reward if reward == -1 else (
                            reward + self.DAMPING_FACTOR * np.max(next_Q, 1))
                        ...
                        agent_reward, agent_performance, std_rewards, std_performance = self.act(sess, 1)
                        print(str(ii) + ' Episode: ' + str(int(itr)) + ' Agent performance: ' + str(
                        ...
                        if agent_performance > self.TOPNOTCH:
                            ...
                            save_path = self.brain.saver.save(sess, '../models/problemA4/model.ckpt')

                    avg_all_mean_reward.append(all_mean_reward)
                    avg_all_std_rewards.append(all_std_rewards)

                    avg_all_std_performance.append(all_std_performance)
                    avg_performance.append(performance)

                    avg_loss.append(loss)

                plt_avg_all_mean_reward = np.mean(avg_all_mean_reward, axis=0)
                plt_avg_performance = np.mean(avg_performance, axis=0)
                plt_avg_all_std_performance = np.mean(avg_all_std_performance, axis=0)
                plt_avg_loss = np.mean(avg_loss, axis=0)

                plt.figure(1, figsize=(14, 10), dpi=50)
                plt.subplot(311)
                plotGraph(data=plt_avg_performance[0:-1],
                          title='Agent Performance (avg 100) - (learning rate ' + str(LEARNING_RATE) + ')',
                          color='b', ylabel='Episode Length', label='Episode Length')
                plt.subplot(312)
                plotGraph(data=plt_avg_all_mean_reward[0:-1],
                          title='Agent Rewards (avg 100) - (learning rate ' + str(LEARNING_RATE) + ')',
                          color='g', ylabel='Rewards', label='rewards')
                plt.subplot(313)
                plotGraph(data=plt_avg_loss[0:-1],
                          title='Agent Training Loss - (learning rate ' + str(LEARNING_RATE) + ')',
                          color='r', ylabel='Bellman Loss', label='loss')
                plt.xlabel('Episodes', fontsize=18)
                if SAVE_PLOT:
                    plt.savefig('../plots/problemA4/plot-avg-' + str(LEARNING_RATE) + '.eps', format='eps', dpi=50)
                else:
                    plt.show()
                ...
                self.brain.saver.restore(sess, '../models/problemA4/model.ckpt')
                ...
                pd.DataFrame(performance_dict).to_csv('../outputs/problemA4-' + str(LEARNING_RATE) + '.csv')
