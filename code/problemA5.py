...
class Brain:
    def __init__(self, liner_type, state_dim, n_actions, LEARNING_RATE):
        ...
        if liner_type:
            self.W = tf.Variable(tf.random_normal([state_dim, n_actions], stddev=0.1), dtype=np.float32,
                                 name='weights')
            ...
            if TRAIN_MODE:
                ...
                for itr in range(N_EPISODES):
                    ...
                    for t in range(self.MAX_EPISODE_LEN):
                        ...
                        Q_target = np.copy(Q)
                        Q_target[0, action[0]] = reward if reward == -1 else (reward + self.DAMPING_FACTOR * np.max(next_Q, 1))
                    ...
                    if itr % STEPS == 0:
                        ...
                        if agent_performance > self.TOPNOTCH:
                            ...
                            save_path = self.brain.saver.save(sess, '../models/problemA5/' + str(
                                HIDDEN_LAYER) + '/model.ckpt')
                ...
                if SAVE_PLOT:
                    plt.savefig('../plots/problemA5/plot-' + str(HIDDEN_LAYER) + str(LEARNING_RATE) + '.eps', format='eps', dpi=50)
                else:
                    plt.show()

                if not self.TOPMODEL_SAVED:
                    save_path = self.brain.saver.save(sess, '../models/problemA5/' + str(HIDDEN_LAYER) + '/model.ckpt')
                    print('Model weights saved in file: ', save_path)

            else:
                # test mode
                print('\n\n---Running in test mode---')
                self.brain.saver.restore(sess, '../models/problemA5/' + str(HIDDEN_LAYER) + '/model.ckpt')
                ...
                pd.DataFrame(performance_dict).to_csv('../outputs/problemA5-' + str(HIDDEN_LAYER) + '.csv')
