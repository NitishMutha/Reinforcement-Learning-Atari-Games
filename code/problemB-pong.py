...
RENDER_AGENT = False
N_EPISODES = 1050000  # 1 milliom
EXPERIENCE_BUFFER = 150000
SPIT_RESULTS = 50000  # 50k
...
class Brain:
    ...
    def get_q_network(self, state, n_actions):
        ...
        conv2flat = tf.reshape(conv3, [-1, 12 * 12 * 64])
        dense = tf.layers.dense(inputs=conv2flat, units=512, activation=tf.nn.relu)
...
class Agent:
    ...
    def preprocess(self, I):
        ...
        if AGENT == AGENT_PONG:
            ...
            I = resize(I, (IMAGE_DIM, IMAGE_DIM), mode='reflect')
            I = rgb2gray(I)
        ...
        with tf.Session() as sess:
            ...
            if TRAIN_MODE:
                ...
                for itr in range(N_EPISODES):
                    ...
                    for t in range(self.MAX_EPISODE_LEN):
                        ...
                        if np.random.rand(1) < self.EPSILON:
                            ...
                            self.EPSILON = self.DAMPING_FACTOR * self.EPSILON
                        ...
                        _, agent_loss, summary = self.brain.train(sess, x=np.reshape(states, (BATCH, -1)),
                                                                  y=Q_target2)
                        ...
                        if STEPS % SPIT_RESULTS == 0:
                            ...
                            agent_reward, agent_performance, std_reward, std_performance, mean_episode_score, std_episode_score = self.act(sess, 30)
                            print(' Agent Frames: ' + str(agent_performance) +
                ...
                np.savez('../models/problemB/' + AGENT + '.npz', final_rewards=final_rewards,
                         frame_counts=frame_counts, loss=loss)
